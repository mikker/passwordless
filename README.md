...
You can access your `User` model through `session.authenticatable`.
...
Passwordless.configure do |config|
  ...
  config.token_generator = Passwordless::UrlSafeBase64Generator.new # Used to generate magic link tokens.
  config.restrict_token_reuse = false # By default a magic link token can be used multiple times.
  ...
  config.timeout_at = lambda { 1.hour.from_now } # How long until a magic link expires.

  config.redirect_to_response_options = {} # any allowed response_options for redirect_to can go in here

  # Default redirection paths
  config.success_redirect_path = '/' # When a user succeeds in logging in.
  config.failure_redirect_path = '/' # When a a login is failed for any reason.
  config.sign_out_redirect_path = '/' # When a user logs out.
end
```

### Customizing token generation

By default Passwordless generates tokens using `SecureRandom.urlsafe_base64` but you can change that by setting `Passwordless.config.token_generator` to something else that responds to `call(session)` eg.:
...
```ruby
Passwordless.configure do |config|
  config.timeout_at = lambda { 2.hours.from_now }
end
```
...
```ruby
Passwordless.configure do |config|
  config.expires_at = lambda { 24.hours.from_now }
end
```
...
This can be turned off with `Passwordless.config.redirect_back_after_sign_in = false` but if you just don't save the previous destination, you'll be fine.
